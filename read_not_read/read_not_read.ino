/*
    LÃMITES por soft, funcionan. y se activa finZero() cuando termina de leer
*/
...
long n = 1;
long m = 0;
//int sheetWidth = 1900;//fijarse si estÃ¡ haciendo algo

//NO HAY HOJA
//configurar estas variables segÃºn dimensiones de hoja a usar
const int minX = 0;
const int maxX = 1000;
const int minY = 300;
const int maxY = 1000;
...
//QUE ES ESTO? variables viejas de botÃ³n de encendido
...
/*factor de conversiÃ³n de pixel a pasos
...
int temperatureLoopCounter = 3; //arranca solo si estÃ¡ en 3 temperatureLoopCounter

int flagFindZero = HIGH;//si pongo LOW evita buscar el cero
int flagEnableMotors = HIGH;

//leds testigos para usos mÃºltiples
...
void loop() {
  //Esperar que la resistencia calienta. - Esta mÃ¡quina no quema.
  ...
  //a partir de acÃ¡ la maquina estÃ¡ lista para Burn - Esta mÃ¡quina no quema.
  burn();
}
...
void findZero() {
  ...
  if (flagEndX == HIGH && flagEndY == HIGH) { //si tocaron ambos fines de carrera ya estÃ¡ en 0,0
    flagFindZero = LOW;
    //jueguito de luces para darme cuenta que llegÃ³ a cero
...
void burn() {
  if ((temperatureLoopCounter > 2) && (flagFindZero == LOW)) {
    ...
    if ((valSensor == HIGH) && (flag == LOW)) {
      ...
      digitalWrite(pinResetZ, HIGH); //el motorZ no estÃ¡ prendido siempre?!?!
    ...
    if ( readSheet == true) { //empieza a Burn
      ...
      if (flag == HIGH) {
        if (posX > 0) {//hacia la izquierda
          /* valEndX = analogRead(endStopX);//LÃMITES MECÃNICOS. leo el sensor
            if (valEndX < thresholdEndStopX) {//si llega al sensor
            posX = 0;                     // cree que llegÃ³ al punto
            counterX= posX;
            } */
          digitalWrite(pinDirX, HIGH);
          if (counterX < posX) {
        ...
        if (posY > 0) {//hacia abajo
          ...
          /* valEndY = analogRead(endStopY);//LÃMITES MECÃNICOS. leo el sensor
            if (valEndY < thresholdEndStopY) {//si llega al sensor
             posY = 0;                     // cree que llegÃ³ al punto
             counterY = posY;
            }  */
          digitalWrite(pinDirY, LOW);
          if (counterY < (-posY)) {
    ...
    if (counterX == abs(posX) && counterY == abs(posY)) {
      ...
      counterY = 0;
      m = n;
      n++;//avanzo una posiciÃ³n en el array
      if (n == arraySize - 1) {
        for (int i = 0; i < 3; i++) {//probando que llegÃ³
          sequenceDown();
        }
        readSheet = false; //terminÃ³ de Burn la hoja
        exitOfMark = true;
        flagEnableRollMotor = true;
        n = 1;
        m = 0;
      ...
      arrayRead(); //busca una nueva posiciÃ³n
    }
  }
}
...
void sequenceUp() {//funciÃ³n para evantar en Z, con sensor
  ...
  while (flagEndZ == LOW) {
    ...
    if (valEndZ > umbralEndStopZ) {
      ...
      flagEndZ = HIGH; //flag en HIGH cuando estÃ¡ en 0
    }
  }
}

void sequenceUp2() { //funciÃ³n antigÃ¼a, sin sensor
  digitalWrite(pinDirZ, LOW);
  for (int i = 0; i < stepsZ; i++) {
    moveMotor(pinStepsZ, speedZ);
  }
...
void arrayRead() {
  /*
     chequear antes de moverme si se puede ir o no a ese punto, segun lÃ­mites de ancho de hoja
     si estÃ¡ fuera de los lÃ­mites paso al siguiente punto (sin actualizar el punto anterior)
     tengo que saber mi posiciÃ³n actual Y hacer el calculo Y ver si estÃ¡ dentro de mi hoja
     si no estÃ¡ dentro de mi hoja n++
  */
  //posX = arraX[n];
  posX = pgm_read_word_near(pixelX + n);      //leo nueva cordenada en X
  while ( posX < minX || posX > maxX) {       //si posX se sale de lo lÃ­mites
    n++;                                      //avanzo una posiciÃ³n
  ...
  while ( posY < minY || posY > maxY) {//si posY se sale de lo lÃ­mites
    n++;                               //avanzo una posiciÃ³n
    posY = pgm_read_word_near(pixelY + n);
    digitalWrite(pinLed2, HIGH);
    delay(500);
  ...
  posY = pgm_read_word_near(pixelY + n);//leo nueva posiciÃ³n en Y
  posX = pgm_read_word_near(pixelX + n);//leo nueva posiciÃ³n en X
...
void sheetSensing() {
  ...
  POTE = analogRead(A4); //usado para calibraciÃ³n
  LDR = analogRead(A7);
